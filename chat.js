...
async function loadUserChats() {
    ...
    try {
        ...
        window.chats = chatsWithDetails.sort((a, b) => 
...
function updateChatsList() {
    ...
    window.chats.forEach(chat => {
        ...
        const previewText = truncateText(chat.lastMessage, 40);
            ...
            <div class="chat-item" data-chat-id="${chat.id}">
                ...
                <div class="chat-item-info">
                    ...
                    <div class="chat-item-preview">
                        <span style="color: #718096; font-size: 0.9rem;">${previewText}</span>
                        ${chat.unreadCount > 0 ? `
                            <span style="
                                background: #f56565;
                                color: white;
    ...
    try {
                    ...
                    " data-user-id="${user.id}">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            <div style="
                                ...
                                font-weight: bold;
                            ">
                                ${window.getUserInitials(user.full_name || user.email)}
                            </div>
                            <div>
        ...
        document.querySelectorAll('.add-user-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const userRow = btn.closest('.user-search-result');
                const userName = userRow.querySelector('div > div:first-child').textContent;
        ...
        resultsContainer.innerHTML = '<p style="color: #f56565; text-align: center; padding: 20px;">Ошибка поиска</p>';
        resultsContainer.style.display = 'block';
    }
}
...
function setupRealtimeUpdates() {
        ...
        .on('postgres_changes', 
            ...
            async (payload) => {
                ...
                if (window.currentChat && payload.new.chat_id === window.currentChat.id) {
                    ...
                    window.chatMessages.push(payload.new);
                    updateChatMessages();
                ...
                if (!window.currentChat || window.currentChat.id !== payload.new.chat_id) {
                    const chat = window.chats.find(c => c.id === payload.new.chat_id);
                    if (chat) {
                        showNewMessageNotification(chat.name, payload.new.content);
                    }
                }
            }
        )
    ...
    console.log('🔔 Подписка на обновления чатов установлена');
}

// Подписка на сообщения конкретного чата
function subscribeToChatMessages(chatId) {
    if (!window.supabaseClient || window.chatSubscription) return;
...
async function markMessageAsRead(messageId) {
    ...
    try {
        ...
        if (!readBy.includes(window.currentUser.id)) {
            readBy.push(window.currentUser.id);
...
function showNewMessageNotification(chatName, messageContent) {
    ...
    if (Notification.permission === "granted") {
        // Создаем уведомление
        const notification = new Notification(`Новое сообщение в "${chatName}"`, {
            body: truncateText(messageContent, 100),
            icon: '/favicon.ico',
            tag: 'chat-message'
        });
        ...
        notification.onclick = function() {
            window.focus();
            // Можно добавить открытие соответствующего чата
        };
        ...
        Notification.requestPermission().then(permission => {
            if (permission === "granted") {
                showNewMessageNotification(chatName, messageContent);
            }
        });
    }
}

// Обновление бейджа с непрочитанными сообщениями
...
function filterChats() {
    ...
    items.forEach(item => {
        ...
        const preview = item.querySelector('.chat-item-preview span:first-child').textContent.toLowerCase();
...
function showChatInfo() {
    ...
    let membersList = 'Участники чата:\n\n';
    window.currentChat.members?.forEach(member => {
        const isYou = member.id === window.currentUser?.id;
        membersList += `${isYou ? '👤 ' : ''}${member.full_name || member.email}${isYou ? ' (Вы)' : ''}\n`;
    });
...
window.addUserToSelection = addUserToSelection;
